...
if(isset($_SESSION["kullanici"]))
{
	...
	$ekle = @mysql_query("INSERT INTO kiralik(baslik,tur,fiyat,marka,yil,saat,detay,resimbir,resimiki,resimuc,resimdort) values ('$baslik' , '$tur ' , '$fiyat ' , '$marka' ,'$yil' ,'$saat' , '$detay','$yeniresimadi', '$yeniresimadiiki','$yeniresimadiuc','$yeniresimadidort')");
	if($ekle)
	{
		function sendMessage(){
			$bak = @mysql_fetch_assoc(@mysql_query("select * from kiralik order by id desc LIMIT 1"));
			$baslik = $bak["baslik"];
		$content = array(
		
			"en" => ''.$baslik.' başlıklı ilanımız KİRALIK İLANLAR kısmına eklenmiştir...'
			);
